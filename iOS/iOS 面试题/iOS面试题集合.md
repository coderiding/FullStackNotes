...
tags: 技术改变世界
abbrlink: 49366
date: 2018-10-26 09:45:41
modified: 2020-10-26 09:53:59
---

### 面试官

##### 162.NSUserDefault可以存储对象吗?
* NSUserDefaults支持的数据类型有：NSNumber（NSInteger、float、double），NSString，NSDate，NSArray，NSDictionary，BOOL.
* NSUserDefaults 本身不支持自定义对象的存储，不过它支持NSData的类型
* 比如一个Student 的类，这个类里有三个属性（学号，姓名，性别），将Student类型变成NSData类型 ，那么就必须实现归档：
	* 在.h 文件中申明 NSCoding 协议，再 在 .m 中实现 encodeWithCoder 方法 和 initWithCoder 方法
...
##### 160. Swift 到底是面向对象还是函数式的编程语言？

* Swift 既是面向对象的，又是函数式的编程语言。
* 说 Swift 是 Object-oriented，是因为 Swift 支持类的封装、继承、和多态，从这点上来看与 Java 这类纯面向对象的语言几乎毫无差别。
* 说 Swift 是函数式编程语言，是因为 Swift 支持 map, reduce, filter, flatmap 这类去除中间状态、数学函数式的方法，更加强调运算结果而不是中间过程。

##### 159.CocoaPods registerNib

```
NSBundle *xibBundle = [NSBundle bundleForClass:[self class]];

[_tableView registerNib:[UINib nibWithNibName:@"MXGiftCell" bundle:xibBundle] forCellReuseIdentifier:@"MXGiftCell"];
```

##### 158.从storyboard中加载控件，在哪个方法能获取到真正的位置大小

* 在 viewDidAppear 或者viewDidLayoutSubviews方法中可以得到；
* 同理，需要改变storyboard中的frame，也必须在这两个方法，在viewDidload中修改无效；
* 百度网盘 1591004714
...
* struct 是值类型，class 是引用类型。（汶：引用类型可以指向同一个内存地址，就会改变同一个内存地址的东西，指向这个内存地址的都会改变。但是值类型不会存在这个问题）
...
* filter函数就是选择集合中符合条件的元素，过滤掉不符合条件的元素
...
* reduce函数可以将一个集合中的所有元素组合起来，生成一个新的值并返回该值
...
- 1、管理方式不同；**（汶：栈区归系统管理，堆区归程序员手动管理）**
- 2、空间大小不同；**（汶：在32位操作系统下，堆区分配了大概4g内存，栈区只有1m，栈区内存大小可以修改）**
- 3、能否产生碎片不同；**（汶：栈区不会产生碎片，因为分配栈内存的时候，是排序的，是先进后出的。堆区会产生碎片，因为堆区分配内存的方式会造成内存空间不连续）**
- 4、生长方向不同；**（汶：堆区内存是向着内存地址增加的方向生长，栈区内存是向着内存地址减少的方向生长）**
- 5、分配方式不同；**（汶：堆区内存只有动态分配，栈区内存可以分为静态分配和动态分配，局部变量的创建，就是静态分配，栈区的动态分配和堆区的动态分配不同，不同点是栈区的动态分配后是由系统自动释放，而堆区动态分配的内存有程序员手动释放）**
- 6、分配效率不同；**（汶：栈的分配效率更高，因为系统有支持栈的快速分配方式给它）**
